...
#define HI_LOG(level, module, fmt, ...)  \
    do {\
        extern HI_BOOL g_bLogOn ;\
        if ((level <= HI_LOG_LEVEL)&&(g_bLogOn)) {\
            fprintf(stdout, module "(%s-%d:%d): " fmt, __FUNCTION__, __LINE__, level, ##__VA_ARGS__);\
        }\
    } while(0)
#endif
...
#define MLOGE(fmt, ...)  HI_LOG(HI_LOG_LEVEL_ERROR, HI_MODULE, fmt, ##__VA_ARGS__)
#define MLOGW(fmt, ...)  HI_LOG(HI_LOG_LEVEL_WARNING, HI_MODULE, fmt, ##__VA_ARGS__)
#define MLOGD(fmt, ...)  HI_LOG(HI_LOG_LEVEL_DEBUG, HI_MODULE, fmt, ##__VA_ARGS__)
#else
